from easyeditor.device_config import EASYEDIT_DEVICE
import sys
import os
...
def _locate_toxic_layer(model, tokenizer, requests, hparams, **kwargs):
        toxic_layer = []
        input = tokenizer([value for pair in requests for value in [pair["target_new"], pair["ground_truth"]]], return_tensors="pt", padding=True, truncation=True).to(EASYEDIT_DEVICE)
        with torch.no_grad():
            outputs = model(**input)
...
def edit(harmful_question, target_new, ground_truth, attack_prompt):
    ...
    model = AutoModelForCausalLM.from_pretrained(hparams.model_name, output_hidden_states=True, torch_dtype=torch_dtype, trust_remote_code=True, device_map=EASYEDIT_DEVICE)
...
def generate(input_text, target_new=None):
    ...
    edit_input_ids = tok.encode(edit_input_text, return_tensors='pt').to(EASYEDIT_DEVICE)
    ...
    if ori_model is None:
        # torch_dtype = torch.float16 if hasattr(hparams, 'fp16') and hparams.fp16 else torch.float32
        torch_dtype = torch.bfloat16
        ori_model = AutoModelForCausalLM.from_pretrained(hparams.model_name, torch_dtype=torch_dtype, trust_remote_code=True, device_map=EASYEDIT_DEVICE)
    
    input_ids = tok.encode(input_text, return_tensors='pt').to(EASYEDIT_DEVICE)

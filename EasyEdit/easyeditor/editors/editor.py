...
class BaseEditor:
    ...
    def __init__(self, hparams: HyperParams):
        ...
        if not hparams.model_parallel and hasattr(hparams, 'device') and hparams.alg_name != 'QLoRA':
            if str(hparams.device) == EASYEDIT_DEVICE:
                self.model.to(EASYEDIT_DEVICE)
                print(f"Set device to {EASYEDIT_DEVICE}")
            else:
                print('rip')
        ...
        if sequential_edit:
            ...
            for i, request in enumerate(tqdm(requests, total=len(requests))):
                ...
                if self.alg_name == 'KN' or self.alg_name == 'GRACE' or self.alg_name == 'WISE':
                    ...
                    # Keep edited weights for inspection/generation.
                    self.model = edited_model
    ...
    def normal_edit(
        ...
        with torch.no_grad():
            for k, v in weights_copy.items():
                nethook.get_parameter(self.model, k)[...] = v.to(EASYEDIT_DEVICE)
    ...
    def generate_edit(
        ...
        **kwargs
        ...
        def text_generate(
            ...
            model_inputs = tok.encode(text, return_tensors="pt").to(EASYEDIT_DEVICE)

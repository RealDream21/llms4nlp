...
def compute_z(
    ...
    target_ids = tok.encode(request["target_new"], return_tensors="pt", add_special_tokens=False).to(EASYEDIT_DEVICE)[0]
    ...
    rewriting_targets = torch.tensor(-100, device=EASYEDIT_DEVICE).repeat(
    ...
    if hasattr(model.config, 'n_embd'):
        delta = torch.zeros((model.config.n_embd,), requires_grad=True, device=EASYEDIT_DEVICE)
    elif hasattr(model.config, 'hidden_size'):
        delta = torch.zeros((model.config.hidden_size,), requires_grad=True, device=EASYEDIT_DEVICE)
    else:

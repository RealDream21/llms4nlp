  ...
  #-> Tuple[AutoModelForCausalLM, Dict[str, Any]]:
    ...
    with torch.no_grad():
        for w_name, upd_m in deltas.items():
            upd_matrix = upd_m.to(EASYEDIT_DEVICE)
...
def execute_AlphaEdit(
    ...
    for request in requests:
        ...
        if (
            ...
            try:
                ...
                z_list.append(torch.from_numpy(data["v_star"]).to(EASYEDIT_DEVICE))
                data_loaded = True
    ...
    for i, layer in enumerate(hparams.layers):
        ...
        upd_matrix = torch.linalg.solve(
                P[i,:,:].to(EASYEDIT_DEVICE) @ (layer_ks.to(EASYEDIT_DEVICE) @ layer_ks.T.to(EASYEDIT_DEVICE) + cache_c[i,:,:].to(EASYEDIT_DEVICE)) + hparams.L2*torch.eye(layer_ks.shape[0], dtype=torch.float,device=EASYEDIT_DEVICE),
                P[i,:,:].to(EASYEDIT_DEVICE) @ layer_ks.to(EASYEDIT_DEVICE) @ resid.T.to(EASYEDIT_DEVICE)
        )
...
def get_cov(
    ...
    return (
        torch.inverse(COV_CACHE[key].to(EASYEDIT_DEVICE)) if inv else COV_CACHE[key].to(EASYEDIT_DEVICE)
    )
